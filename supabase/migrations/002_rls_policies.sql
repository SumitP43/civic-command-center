...
CREATE OR REPLACE FUNCTION auth.is_department_admin()
RETURNS BOOLEAN AS $$
  SELECT EXISTS (SELECT 1 FROM profiles WHERE id = auth.uid() AND role = 'department_admin');
$$ LANGUAGE sql SECURITY DEFINER STABLE;

-- Get officer's department_id
CREATE OR REPLACE FUNCTION auth.officer_department_id()
RETURNS UUID AS $$
  SELECT department_id FROM officers WHERE profile_id = auth.uid();
$$ LANGUAGE sql SECURITY DEFINER STABLE;
...
CREATE POLICY profiles_select_dept_admin ON profiles
  FOR SELECT USING (
    auth.is_department_admin() AND (
      id IN (
        SELECT o.profile_id FROM officers o
        WHERE o.department_id = auth.officer_department_id()
      )
      OR role = 'citizen'
    )
  );
...
CREATE POLICY officers_select_dept ON officers
  FOR SELECT USING (
    auth.is_department_admin() AND department_id = auth.officer_department_id()
  );
...
CREATE POLICY officers_insert_admin ON officers
  FOR INSERT WITH CHECK (auth.is_super_admin() OR auth.is_department_admin());

CREATE POLICY officers_update_admin ON officers
  FOR UPDATE USING (
    auth.is_super_admin()
    OR (auth.is_department_admin() AND department_id = auth.officer_department_id())
  );
...
CREATE POLICY complaints_select_officer ON complaints
  FOR SELECT USING (
    auth.user_role() = 'officer' AND id IN (
...
CREATE POLICY media_select_officer ON complaint_media
  FOR SELECT USING (
    auth.user_role() = 'officer' AND complaint_id IN (
      SELECT ca.complaint_id FROM complaint_assignments ca
      JOIN officers o ON o.id = ca.officer_id
      WHERE o.profile_id = auth.uid() AND ca.is_active = TRUE
    )
...
CREATE POLICY media_select_dept ON complaint_media
  FOR SELECT USING (
    auth.is_department_admin() AND complaint_id IN (
      SELECT id FROM complaints WHERE department_id = auth.officer_department_id()
    )
  );
...
CREATE POLICY assignments_select_dept ON complaint_assignments
  FOR SELECT USING (
    auth.is_department_admin() AND officer_id IN (
      SELECT id FROM officers WHERE department_id = auth.officer_department_id()
    )
  );
...
CREATE POLICY assignments_insert_dept ON complaint_assignments
  FOR INSERT WITH CHECK (
    auth.is_department_admin() OR auth.is_super_admin()
  );

CREATE POLICY assignments_update ON complaint_assignments
  FOR UPDATE USING (
    officer_id IN (SELECT id FROM officers WHERE profile_id = auth.uid())
    OR auth.is_department_admin()
...
CREATE POLICY ai_analysis_select_admin ON ai_analysis
  FOR SELECT USING (auth.is_super_admin() OR auth.is_department_admin());
